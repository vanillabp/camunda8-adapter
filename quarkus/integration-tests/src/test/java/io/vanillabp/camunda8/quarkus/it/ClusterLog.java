 ...
 * {@code target}.
 * <p>
 * The application of these tests already writes its own log, because it runs in a forked
 * JVM. What that log says stops at the adapter: whether the cluster ever handed a job out
 * is the other half of every notification which never arrived, and that half goes with
 * the container when the run ends. The file is what the build uploads next to the test
 * reports when it fails, and its name carries {@code application} because that is the
 * shape of file the upload collects.
 * <p>
 ...
 * <p>
 * The Spring Boot integration tests carry a writer of their own. The two modules share no
...
final class ClusterLog {
  ...
  /**
   * Numbers the containers apart. Both test classes of this module run in the same fork,
   * and each of them starts a cluster with the same image.
   */
