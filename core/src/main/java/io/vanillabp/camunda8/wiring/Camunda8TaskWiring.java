...
public final class Camunda8TaskWiring {
  ...
  /**
   * Writes VanillaBP's own cancel listener beside every listener somebody modelled and this
   * application serves, so the method serving the listener hears it when the element is
   * canceled.
   * <p>
   * A modelled listener fires at the moment the modeller picked and at no other. An
   * interrupting boundary event or a terminating end event takes the element away without that
   * moment ever arriving, so the application is never told. VanillaBP writes one cancel listener
   * per served listener, carrying the SAME job type, which puts the cancellation in front of the
   * method which already serves that listener. One listener with one job and one method, exactly
   * as everywhere else.
   * <p>
   * A listener the modeller put on the cancel moment itself is left alone. It already hears
   * that moment, and a second listener would report it to the same method twice.
   * <p>
   * What VanillaBP can write depends on the element. A Camunda-managed user task carries a
   * {@code canceling} task listener on every release line. Every other element needs a
   * {@code cancel} execution listener, which arrived with 8.10, so on the older lines those
   * elements cannot be told at all. They are returned, and the startup report names them
   * rather than leaving the gap to be found at runtime.
   * <p>
   * Order counts within one listener event and nowhere else, so a cancel listener moves
   * nothing a modeller wrote for the {@code creating} or {@code start} moment of the same
   * element.
   * <p>
   * Why a listener knows these two events and no others is decision 32 in the repository's
   * DECISIONS.md.
   *
   * @param model The BPMN model of one file, modified in place
   * @param servedListeners The listeners of this process a {@code @WorkflowTask} method serves
   * @param jobTypeAsTheClusterKnowsIt Turns the plain task definition of a listener into the
   *          job type the model carries, which is the prefixed one where the workflow module
   *          avoids name clashes that way
   * @return The served listeners this release line cannot tell about a cancellation, in the
   *         order they were given
   */
  public static List<Camunda8Listeners.ModelledListener> addCancelListenersFor(
      final BpmnModelInstance model,
      final List<Camunda8Listeners.ModelledListener> servedListeners,
      final UnaryOperator<String> jobTypeAsTheClusterKnowsIt) {

    final var withoutACancellation = new LinkedList<Camunda8Listeners.ModelledListener>();
    for (final var listener : servedListeners) {
      if (Camunda8Listeners.isACancellation(listener)) {
        continue;
      }
      ...
      final var jobType = jobTypeAsTheClusterKnowsIt.apply(listener.taskDefinition());
      if ((baseElement instanceof UserTask userTask) && (userTask
          .getSingleExtensionElement(ZeebeUserTask.class) != null)) {
        addCancelTaskListener(userTask, jobType);
        continue;
      }
      if (!Camunda8CancelListeners.anyElementCanReportItsCancellation()) {
        withoutACancellation.add(listener);
        continue;
      }
      final var listeners = executionListenersOf(baseElement);
      final var alreadyWired = listeners
          .getExecutionListeners()
          .stream()
          .anyMatch(
              written -> jobType.equals(written.getType()) && Camunda8Listeners.CANCELING_AN_ELEMENT
                  .equals(String.valueOf(written.getEventType())));
      if (alreadyWired) {
        continue;
      }
      Camunda8CancelListeners.addCancelListener(listeners, jobType);
    }
    return withoutACancellation;

  }

  /**
   * Writes a <code>canceling</code> task listener of the given job type to a Camunda-managed
   * user task, unless the model already carries it (re-wiring an already-processed model).
   *
   * @param task The user task
   * @param jobType The job type of the served listener
   */
  private static void addCancelTaskListener(
      final UserTask task,
    ...
    if (task.getSingleExtensionElement(ZeebeTaskListeners.class) != null) {
      ...
      final var alreadyWired = taskListeners
          ...
          .stream()
          .anyMatch(
              written -> jobType.equals(written.getType()) && ZeebeTaskListenerEventType.canceling
                  .equals(written.getEventType()));
      if (alreadyWired) {
        return;
      }
    } else {
    ...
    final var cancelListener = task
        .getModelInstance()
        .newInstance(ZeebeTaskListener.class);
    cancelListener.setEventType(ZeebeTaskListenerEventType.canceling);
    cancelListener.setType(jobType);
    cancelListener.setRetries(Camunda8Listeners.CANCEL_LISTENER_RETRIES);
    taskListeners.addChildElement(cancelListener);

  }

  /**
   * The execution listeners of an element, creating the container and the extension elements
   * around it where the model carries neither.
   *
   * @param element The element
   * @return The container to add a listener to
   */
